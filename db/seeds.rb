...
avengers = Server.create!(server_name: "Avengers", server_image: "true", description: "Saving the world", private: true, admin_id: captain_america.id)

member = ServerUser.create!(user_id: spiderman.id, server_id: avengers.id)
member = ServerUser.create!(user_id: thor.id, server_id: avengers.id)
member = ServerUser.create!(user_id: hulk.id, server_id: avengers.id)
member = ServerUser.create!(user_id: iron_man.id, server_id: avengers.id)
member = ServerUser.create!(user_id: captain_america.id, server_id: avengers.id)
member = ServerUser.create!(user_id: dr_strange.id, server_id: avengers.id)
member = ServerUser.create!(user_id: hawkeye.id, server_id: avengers.id)
member = ServerUser.create!(user_id: black_widow.id, server_id: avengers.id)
demo_member = ServerUser.create!(user_id: demo.id, server_id: avengers.id)

superheroes = Channel.create!(channel_name: "Superhero stuff", description: "Superhero talk",  server_id: avengers.id)

superheroes.messages.create!(body: "Whoa I love this new chat, now we can all stay in touch", user_id: spiderman.id, channel_id: superheroes.id)
superheroes.messages.create!(body: "It still won't allow you to be worthy of my hammer", user_id: thor.id, channel_id: superheroes.id)
superheroes.messages.create!(body: "HULK SMASH!!!", user_id: hulk.id, channel_id: superheroes.id)
superheroes.messages.create!(body: "You like it? built it myself", user_id: iron_man.id, channel_id: superheroes.id)
superheroes.messages.create!(body: "Calm down Hulk", user_id: captain_america.id, channel_id: superheroes.id)
superheroes.messages.create!(body: "So are we going to chat all day or save the world?", user_id: dr_strange.id, channel_id: superheroes.id)
superheroes.messages.create!(body: "Well I have target practice soon, my bow is getting rusty. Talk later.", user_id: hawkeye.id, channel_id: superheroes.id)
...
member = ServerUser.create!(user_id: leia.id, server_id: jedi.id)
member = ServerUser.create!(user_id: rey.id, server_id: jedi.id)
member = ServerUser.create!(user_id: darth_vader.id, server_id: sith.id)
member = ServerUser.create!(user_id: palpatine.id, server_id: sith.id)
member = ServerUser.create!(user_id: kylo.id, server_id: sith.id)
demo_member = ServerUser.create!(user_id: demo.id, server_id: jedi.id)
demo_member = ServerUser.create!(user_id: demo.id, server_id: sith.id)
...
falcon.messages.create!(body: "Han why did you even create this chat you're not a Jedi, and why did you name it after your ship?", user_id: luke.id, channel_id: falcon.id)
falcon.messages.create!(body: "I was bored, and it's the best ship in the galaxy.", user_id: han.id, channel_id: falcon.id)
falcon.messages.create!(body: "ughhhh here we go again.", user_id: leia.id, channel_id: falcon.id)
falcon.messages.create!(body: "Does anyone know who my parents are?", user_id: rey.id, channel_id: falcon.id)
bad_guys.messages.create!(body: "So what is our plan?", user_id: darth_vader.id, channel_id: bad_guys.id)
bad_guys.messages.create!(body: "We must get as many Jedi as we can to turn to the dark side.", user_id: palpatine.id, channel_id: bad_guys.id)
bad_guys.messages.create!(body: "I can turn Rey, I know it", user_id: kylo.id, channel_id: bad_guys.id)
...
gryffindor = Server.create!(server_name: "Gryffindor", server_image: "true", description: "Gryffidor House", private: true, admin_id: harry.id)
slytherin = Server.create!(server_name: "Slytherin", server_image: "true", description: "Slytherin House", private: true, admin_id: draco.id)
professors = Server.create!(server_name: "Professors", server_image: "true", description: "Proffesors of Hogwarts", private: true, admin_id: dumbledore.id)
villains = Server.create!(server_name: "Villains", server_image: "true", description: "We are villains", private: true, admin_id: voldemort.id)

member = ServerUser.create!(user_id: harry.id, server_id: gryffindor.id)
member = ServerUser.create!(user_id: ron.id, server_id: gryffindor.id)
member = ServerUser.create!(user_id: hermoine.id, server_id: gryffindor.id)
member = ServerUser.create!(user_id: draco.id, server_id: slytherin.id)
member = ServerUser.create!(user_id: crabbe.id, server_id: slytherin.id)
member = ServerUser.create!(user_id: goyle.id, server_id: slytherin.id)
member = ServerUser.create!(user_id: dumbledore.id, server_id: professors.id)
member = ServerUser.create!(user_id: mcgonagall.id, server_id: professors.id)
member = ServerUser.create!(user_id: snape.id, server_id: professors.id)
member = ServerUser.create!(user_id: voldemort.id, server_id: villains.id)
member = ServerUser.create!(user_id: lucius.id, server_id: villains.id)
member = ServerUser.create!(user_id: bellatrix.id, server_id: villains.id)
demo_member = ServerUser.create!(user_id: demo.id, server_id: gryffindor.id)
demo_member = ServerUser.create!(user_id: demo.id, server_id: slytherin.id)
demo_member = ServerUser.create!(user_id: demo.id, server_id: professors.id)
demo_member = ServerUser.create!(user_id: demo.id, server_id: villains.id)

lion = Channel.create!(channel_name: "Lions", description: "Best house at Hogwarts",  server_id: gryffindor.id)
snake = Channel.create!(channel_name: "Snakes", description: "Don't underestimate us",  server_id: slytherin.id)
wizards = Channel.create!(channel_name: "Best Professors", description: "Best professors at Hogwarts",  server_id: professors.id)
death_eaters = Channel.create!(channel_name: "Voldemorts Army", description: "The most evil wizards in the world",  server_id: villains.id)

lion.messages.create!(body: "Guys I think I know what Voldemort is up to, meet me at Hagrids Hut", user_id: harry.id, channel_id: lion.id)
lion.messages.create!(body: "Be careful Harry, I'll meet you there now, lets go Ron!", user_id: hermoine.id, channel_id: lion.id)
lion.messages.create!(body: "I'll be there soon, this makes me nervous.", user_id: ron.id, channel_id: lion.id)
snake.messages.create!(body: "Crabbe, Goyle... I saw Harry walking to Hagrids hut. He must be up to something, let's go snoop around", user_id: draco.id, channel_id: snake.id)
snake.messages.create!(body: "I'll be right over Draco, this is going to be good.", user_id: crabbe.id, channel_id: snake.id)
snake.messages.create!(body: "I'm going to get something to eat first.", user_id: goyle.id, channel_id: snake.id)
wizards.messages.create!(body: "We need to figure out Voldemorts next move and we have to make sure Harry and the other students aren't in danger", user_id: dumbledore.id, channel_id: wizards.id)
wizards.messages.create!(body: "What do you suppose we do?", user_id: mcgonagall.id, channel_id: wizards.id)
wizards.messages.create!(body: "Harry Potter is a fool if he thinks he'll defeat Voldemort", user_id: snape.id, channel_id: wizards.id)
